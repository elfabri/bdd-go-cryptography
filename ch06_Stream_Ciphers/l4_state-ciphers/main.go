/*
Update the crypt function.
It should print this message after each character
is sent to the result channel:

Crypted byte: NUM

Where NUM is the number of bytes
that have been encrypted so far, starting at 1.
*/
...
func crypt(textCh, keyCh <-chan byte, result chan<- byte) {
	...
	for {
        ...
        enc := textChar ^ keyChar
		result <- enc
        fmt.Printf("Crypted byte: %v\n", enc)
	}
}
